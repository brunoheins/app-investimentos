...
def ler_planilha(nome_aba):
    ...
    try:
        ...
        try:
            sheet = client.open("App_Investimentos").worksheet(nome_aba)
        except:
            return pd.DataFrame()
        dados = sheet.get_all_records()
        return pd.DataFrame(dados)
    except Exception as e:
        st.error(f"Erro ao ler planilha {nome_aba}: {e}")
        return pd.DataFrame()
...
def salvar_configuracao(email, dados_dict):
    ...
    try:
        ...
        try:
            ...
            sheet = client.open("App_Investimentos").add_worksheet(title="Configuracao", rows=100, cols=12)
            headers = ["Email", "RF", "RV", "RV_Brasil", "RV_Exterior", "BR_Acoes", "BR_FIIs", "EX_Stocks", "EX_REITs", "EX_ETFs"]
        ...
        if not df_all.empty and 'Email' in df_all.columns:
            ...
            df_filtered = df_all[df_all['Email'] != email]
        else:
            df_filtered = pd.DataFrame(columns=["Email", "RF", "RV", "RV_Brasil", "RV_Exterior", "BR_Acoes", "BR_FIIs", "EX_Stocks", "EX_REITs", "EX_ETFs"])
            
        nova_linha = [email, dados_dict['RF'], dados_dict['RV'], dados_dict['RV_Brasil'], dados_dict['RV_Exterior'], 
                      dados_dict['BR_Acoes'], dados_dict['BR_FIIs'], dados_dict['EX_Stocks'], dados_dict['EX_REITs'], dados_dict['EX_ETFs']]
        
        dados_finais = [["Email", "RF", "RV", "RV_Brasil", "RV_Exterior", "BR_Acoes", "BR_FIIs", "EX_Stocks", "EX_REITs", "EX_ETFs"]]
        if not df_filtered.empty:
            for _, r in df_filtered.iterrows():
                dados_finais.append([
                    r.get('Email', ''), r.get('RF', 0), r.get('RV', 0), r.get('RV_Brasil', 0), r.get('RV_Exterior', 0),
                    r.get('BR_Acoes', 0), r.get('BR_FIIs', 0), r.get('EX_Stocks', 0), r.get('EX_REITs', 0), r.get('EX_ETFs', 0)
                ])
...
if 'email' not in st.session_state:
    st.session_state.email = "usuario@exemplo.com" # Ajuste conforme seu login real

st.sidebar.title("Menu Principal")
menu_selecionado = st.sidebar.selectbox("Navegação", ["⚙️ Configuração da Carteira", "Outras Telas"])

if menu_selecionado == "⚙️ Configuração da Carteira":
    st.title("⚙️ Central de Configuração da Carteira")
    
    # Inicializa o controle das sub-abas por botões
    if 'config_aba_ativa' not in st.session_state:
        st.session_state.config_aba_ativa = "Metas"

    col_btn1, col_btn2 = st.columns(2)
    
    with col_btn1:
        tipo_btn1 = "primary" if st.session_state.config_aba_ativa == "Metas" else "secondary"
        if st.button("🎯 Metas de Alocação", use_container_width=True, type=tipo_btn1):
            st.session_state.config_aba_ativa = "Metas"
            st.rerun()
            
    with col_btn2:
        tipo_btn2 = "primary" if st.session_state.config_aba_ativa == "Ativos" else "secondary"
        if st.button("📋 Ativos e Pesos por Categoria", use_container_width=True, type=tipo_btn2):
            st.session_state.config_aba_ativa = "Ativos"
            st.rerun()

    st.markdown("---")

    # ==========================================
    # SEÇÃO 1: METAS DE ALOCAÇÃO
    # ==========================================
    if st.session_state.config_aba_ativa == "Metas":
    ...
    # ==========================================
    # SEÇÃO 2: ATIVOS E PESOS POR CATEGORIA
    # ==========================================
    else:
    ...
    st.title("Outras telas")
    st.write("Demais seções do aplicativo.")
